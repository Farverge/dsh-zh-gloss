...
- ✅ **官方不渲染的信息**——设置→插件列表的 182 个插件描述（DOM 注入，覆盖 loader 注册表 id + Agent 预设组合行 + 运行时挂载行）

外加：**应用内开关**（设置→通用设置→语言行左侧，「汉化：开/关」热启停）、
**词典热更新**（改 JSON 刷新即生效）、**官方补译自动让位**（追赶自愈）、
**首屏缓存**（无英文闪烁）、**卸载逐字节还原**（139 项离线自测 + 45 项真实浏览器推演保证）。
...
- 针对 `@deepseek-ai/dsh@0.1.6-alpha.1` 验证（轨迹徽章/详情面板/命令目录/权限预设名/enter 行为等官方已陆续内置，插件词条自动让位）；官方升级后
